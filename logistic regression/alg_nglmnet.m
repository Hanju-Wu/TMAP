function [funv_array, xopt, resi_array, num_iter, cput, ssa, stats] = alg_nglmnet( x0, data, model, opts)
...
xopt = x;
cput = toc(tstart);
used = max(1,min(k,maxit-1));
funv_array = funv_array(1:used);
resi_array = resi_array(1:used);
time_array = time_array(1:used);
mvp_array = mvp_array(1:used);
