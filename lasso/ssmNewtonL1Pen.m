function [x, out] = ssmNewtonL1Pen(A,b,N,mu,opts)
...
for iter = 1:maxit
    ...
    [d(~ind),iterd,flag,~,~,~,relres] = ...
        mypcgw(A,r(~ind),CG_tol,CG_maxit,[],0,sig,tau,ind,N);
    ...
    %if resxz < res%resFac*max(resnp)
    fprintf('res: %e, resxz: %e\n', res, resxz);
    ...
    %         %[Ftxz, resxz, yxz] = comp_resR(xz,gradxz);
    %         [Ftxt, rest, yxt, gradt] = comp_res(xz);
    %         reslist(di) = rest;
    % 
    %         f1 = func(xz);
    %         flist(di) = f1;
    %     end
    %     % fig = figure(1); plot(step,reslist-res,'-x');
    %     % xlabel('step size t');
    %     % ylabel('$\|F(x+td)\|-\|F(x)\|$','Interpreter','latex');
    %     % print(fig, '-depsc',strcat('lasso-res1-',fna,'.eps'))
    %     % 
    %     % fig = figure(2); plot(step, flist-f0,'-x');
    %     % xlabel('step size t');
    %     % ylabel('$\psi(x+td)-\psi(x)$','Interpreter','latex');
    %     % print(fig, '-depsc',strcat('lasso-fun1-',fna,'.eps'))
    %     % %return
    % end
...
%         resRatiop = res/resp; 
   % end
    ...
    if record>=1 && (cstop || ...
        ...
        if opts.crit == 2 & mu <= (1+1e-10)*muf
            fprintf(str_num, iter, mu, res/tau, (fiter - fopt)/max(1,abs(fopt)), ...
                Ftxzd, rhs, ratio, tau,lambda, flag, iterd, relres, ddflag);
        else
