...
addpath(genpath('../../SuiteLasso-main'));
...
% for j = 1:1  
    mu = data.mu(j);
    % mu = 1e-2;
    [x,b,~,R,Rt]    = createSignal(N,M,K,dyna,sigma,U,data.seed(j));
    Amap               = @(y) R(U(y));
    Atmap              = @(y) Ut(Rt(y));
    % A = Amap(eye(N,N));

    % optimality measure
    gradx  = @(x) x-Atmap(Amap(x)-b);
    ...
    opts2.crit       = 1;
    opts2.tol        = 1e-14;
    [xopt,outopt]    = ssmNewtonL1Pen(Amap,Atmap,b,N,mu,opts2);
    fopt = mu*norm(xopt,1)+0.5*norm(Amap(xopt)-b)^2;
    ...
    for i = 1:length(tol) 
        opts2.crit       = 1;
        opts2.tol        = tol(i);
        [xtol,outtol] = ssmNewtonL1Pen(Amap,Atmap,b,N,mu,opts2);
       % fsnf = mu*norm(xtol,1)+0.5*norm(A(xtol)-b)^2;
        ...
        [x3,out3]     = ssmNewtonL1Pen(Amap,Atmap,b,N,mu,opts2);
        ...
        % Ainput.Amap = Amap;
        % Ainput.ATmap = ATmap;
        % [obj,y,xi,x,info,runhist] = Classic_Lasso_SSNAL(Ainput,b,N,mu,opts);
        % time(5,i) = time(5,i) + info.time;
        
        opts2.maxit      = 10000;
        opts2.mm = 2;
        [x7,out7]     = ssmNewtonL1Pen_LBFGSH(Amap,Atmap,b,N,mu,opts2);
        Acount(7,i) = Acount(7,i) + out7.Acalls;
        time(7,i) = time(7,i) + out7.time;
        
        opts2.maxit      = 10000;
        opts2.mm = 1;
        [x8,out8]     = ssmNewtonL1Pen_LBFGSH(Amap,Atmap,b,N,mu,opts2);
        Acount(8,i) = Acount(8,i) + out8.Acalls;
        time(8,i) = time(8,i) + out8.time;

        [x9,out9]     = tmap(Amap,Atmap,b,N,mu,opts2);
        Acount(9,i) = Acount(9,i) + out9.Acalls;
        time(9,i) = time(9,i) + out9.time;
